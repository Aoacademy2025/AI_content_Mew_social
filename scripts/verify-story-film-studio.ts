...
const presenterUpload = readFileSync("src/lib/story-film-presenter-upload.server.ts", "utf8");
...
assert.match(presenterUploadRoute, /uploadStoryFilmPresenter\(request, user\.id\)/);
assert.match(presenterUpload, /probeVideoMedia\(outputPath\)/);
assert.match(presenterUpload, /!metadata\.hasAudio/);
assert.match(presenterUpload, /registerStoryFilmPresenterAsset/);
...
assert.match(workbench, /sceneKeys: finalRepairSceneKeys/);
assert.match(workbench, /repairLayer/);
assert.match(workbench, /<EditorialDesk/);
assert.match(workbench, /Hero editorial engine/);
assert.match(workbench, /Headline Hook/);
assert.match(workbench, /subtitleStylePreset/);
assert.match(workbench, /subtitleTextEffect/);
assert.match(workbench, /<VisualQaChecklist/);
assert.match(workbench, /editorial: editorialTarget/);
assert.match(workbench, /ฟังเสียงมิวพร้อมเพลง/);
assert.match(workbench, /music\.volume = 0\.12/);
assert.match(workbench, /สร้าง Final Preview/);
...
console.log("ok: Studio supports Hero subtitles/Headline, Final Cut setup, Visual QA, and selective repair");
console.log("ok: narration and selected music can be previewed together without a render job");
