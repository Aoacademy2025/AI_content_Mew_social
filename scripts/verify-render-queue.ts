...
async function main() {
  ...
  // store.getRenderJob is now a NARROW poll projection (PERF-APP-2: it no longer hydrates
  // `payload`/reservation fields). This test inspects full-row fields (reservedQuota,
  // attempts, phase, cancelRequested, startedAt/heartbeatAt), so read the full row directly.
  const full = (id: string) => prisma.renderJob.findUnique({ where: { id } });
  ...
  const burnJob = await full(burn.id);
  ...
  const after = await full(stale.id);
  ...
  const deadAfter = await full(dead.id);
  ...
  const deadAfter2 = await full(dead.id);
  ...
  await store.sweepDeadRenderJobs(90_000);
  const retryQueued = await full(retry.id);
  ...
  const retryDone = await full(retry.id);
  ...
  const drBefore = await full(dr.id);
  ...
  const drAfter = await full(dr.id);
  ...
  const raceAfter = await full(race.id);
  ...
  const supAAfter = await full(supA.id);
  const supANewerAfter = await full(supANewer.id);
  ...
  const supNewestAfter = await full(supNewest.id);
  ...
  const supOtherAfter = await full(supOther.id);
  ...
  const supRunningAfter = await full(supRunning.id);
