...
| `SUBTITLE_ACOUSTIC_MIN_VERIFIED_SHARE` | `0.9` | Verified-word share at which a partial clock may replace a remote forced alignment; parsed once at startup, clamped 0.5–1 |
...
`apply` can replace an estimated clock with a partial acoustic clock; unsupported/weak words are bounded by verified neighbours and labelled in `uncertainRanges`. Their cards are merged into phrases using original text slices. A partial result preserves existing fully aligned timing unless it verified at least `SUBTITLE_ACOUSTIC_MIN_VERIFIED_SHARE` of the words, or every uncertain range is exactly one Thai repetition mark (ๆ), bounded by verified words within 1.5 seconds. Either qualification may replace remote forced alignment in apply mode — a remote alignment carries no acoustic evidence of its own and has rendered production clips seconds early while reporting itself aligned; provider-supplied alignment (ElevenLabs) and upload transcription remain protected. Every selected partial clock records `partial_forced_alignment` and retains warning status through export. Complete local alignment may replace one only under `apply`. Partial clocks retain warning-quality provenance. No local error stops rendering.

Every projection — shadow included — also records `disagreementMaxMs` and `disagreementMedianMs`: how far the clock that was already rendering sat from the acoustic one, over verified words only, reported on `subtitle_acoustic_done` and persisted in the evidence.
