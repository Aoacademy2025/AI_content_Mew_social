...
`apply` can replace an estimated clock with a partial acoustic clock; unsupported/weak words are bounded by verified neighbours and labelled in `uncertainRanges`. Their cards are merged into phrases using original text slices. A partial result never replaces an existing fully aligned clock. Complete local alignment may replace one only under `apply`. Partial clocks retain warning-quality provenance. No local error stops rendering.
