...
const URL_RE = /^\/api\/renders\/[\w.-]+\.mp4$/;
...
check("rejects bare dot-dot segment", !URL_RE.test("/api/renders/.."));
check("rejects bare dot segment", !URL_RE.test("/api/renders/."));
check("rejects non-mp4", !URL_RE.test("/api/renders/evil.sh"));
