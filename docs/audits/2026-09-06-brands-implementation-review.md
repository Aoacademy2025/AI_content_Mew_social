...
- Unknown Gemini voices, unavailable ElevenLabs plan/key access, inaccessible private Hero voices and removed/foreign logo references are rejected without partially committing the setup. Named voice pickers preserve the selected voice on provider lookup failure. ElevenLabs key/catalog reachability remains governed by the existing provider API/render validation; it is not an offline guarantee.
...
- Final checks ran in the isolated Orca worktree after merging `a4700eba` from main. Production build/typecheck pass without any ignored local operations scripts. The new acoustic-subtitle checks from main also pass.
