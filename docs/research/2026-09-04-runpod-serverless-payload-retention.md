...
- In the previously pinned official `runpod==1.10.0` source, the default `RunPodLogger` level is `DEBUG`, and the job runner logs handler/generator/return output at DEBUG. A generated base64 WAV can therefore enter the 90-day endpoint log unless the deployed endpoint explicitly overrides the level. The hardened candidate updates to `runpod==1.12.0`, whose serverless path retains the same explicit environment-controlled logger boundary, and sets `RUNPOD_LOG_LEVEL=INFO` in both the immutable image and required staging specification. [v1.10.0 logger](https://github.com/runpod/runpod-python/blob/v1.10.0/runpod/serverless/modules/rp_logger.py), [v1.10.0 job runner](https://github.com/runpod/runpod-python/blob/v1.10.0/runpod/serverless/modules/rp_job.py), [v1.12.0 logger](https://github.com/runpod/runpod-python/blob/v1.12.0/runpod/serverless/modules/rp_logger.py)
