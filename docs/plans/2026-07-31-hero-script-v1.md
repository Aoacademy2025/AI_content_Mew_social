...
- **Nav placement:** "เขียนสคริปต์ AI" moved to sit immediately BEFORE Video Editor in `src/components/layout/sidebar.tsx`, in BOTH the user nav (`userNavItems`) and the admin nav (`adminStudioItems`) — the product owner's account is ADMIN, so the item must appear there too, same allowlist gate and "ใหม่" badge (writing comes before editing in the user flow).
- **Internal-beta account allowlist (fail-closed):** Hero Script is gated to allowlisted accounts only, enforced in three layers — nav (sidebar item hidden unless allowlisted, in both the user and admin nav sections), page (`/hero-script` server-redirects non-allowlisted users to `/dashboard`), and API (all 11 `/api/brand-profiles*` + `/api/scripts*` routes return 403 `FEATURE_LOCKED` via the shared `requireHeroScriptUser()` gate in `src/lib/hero-script.server.ts`). The matcher lives in `src/lib/hero-script-access.ts` and reads `HERO_SCRIPT_ALLOWED_EMAILS` (comma-separated, case-insensitive; `@`-prefixed entries are ANCHORED domain matches ONLY — exact domain or a real subdomain of it (`emailDomain === X || emailDomain.endsWith("." + X)`); everything else is an exact email match). There is deliberately NO TLD-agnostic / "starts-with" fallback for TLD-less entries: an earlier version of that convenience was itself an unanchored bypass (anyone owning `aoacademy.evilhacker.io` would have passed a `@aoacademy` entry). The consequence is deliberate — **allowlist entries must be exact emails or FULL domains including the TLD**; a TLD-less entry like `@aoacademy` matches only the literal domain "aoacademy", not "aoacademy.com". An empty/unset env disables the feature for EVERYONE, matching ADR-0003's fail-closed convention — **prod must set `HERO_SCRIPT_ALLOWED_EMAILS` explicitly at deploy**, confirmed prod value (2026-07-31): `HERO_SCRIPT_ALLOWED_EMAILS=duckyhero@gmail.com,info.aoacademy@gmail.com,@aoacademy.co` (owner + team Gmail + team domain, confirmed from git authors and team records) — or no one (not even the product owner) will see the feature. Allowlisted users still follow their normal plan limits — this is a visibility/access gate, not a bypass.
- **Mobile-responsive pass:** deferred to a separate commit (not bundled with this amendment).
- **Model-provider question — DECIDED 2026-07-31:** Hero Script's LLM calls run on **OpenRouter (GPT-5.6 `luna`/`terra`) for the internal beta**, behind an env provider switch whose **code default stays `gemini`**. Details below.

### Provider switch: Hero Script LLM on OpenRouter (decision date 2026-07-31)

**Env matrix** (only Hero Script consults these — the app's other 11 Gemini call sites are untouched):

| Env | Values / default | Effect |
|---|---|---|
| `HERO_SCRIPT_PROVIDER` | `gemini` \| `openrouter` — **code default `gemini`** (unset, blank, or an unknown value → `gemini` + a one-time server warning) | Which provider the Hero Script LLM seam calls |
| `HERO_SCRIPT_MODEL_FAST` / `HERO_SCRIPT_MODEL_PRO` | `gemini-2.5-flash` / `gemini-pro-latest` | Gemini model ids (unchanged) |
| `HERO_SCRIPT_OR_MODEL_FAST` / `HERO_SCRIPT_OR_MODEL_PRO` | `openai/gpt-5.6-luna` / `openai/gpt-5.6-terra` | OpenRouter model slugs |
| `OPENROUTER_API_KEY` | server credential, required when provider = `openrouter` | There is **no OpenRouter BYOK** — one server key serves every user |

The two model-id env pairs are deliberately separate: a Gemini model id is not a routable OpenRouter slug, so switching providers must never inherit the other one's override. Every Hero Script call site (analyze, niche-ideas, ideas, hooks, generate, regen-section) reaches the model through ONE seam — `heroScriptGenerateText()` in `src/lib/hero-script.server.ts`, called from `generateValidatedJson` — so the switch needs no per-route code. The client is `src/lib/openrouter.ts` (OpenAI-compatible `POST /api/v1/chat/completions`, 120s per-attempt timeout, 3 attempts retrying only 429/5xx/network, `temperature: 0`, `max_tokens` ← `maxOutputTokens`) — the same discipline as `geminiGenerateText`, because the two are interchangeable behind that seam.

**Metering (the cost bearer changes):** under `openrouter` the **server** pays for every Hero Script call — including calls from users who are BYOK for Gemini. So `resolveLlmTriad` (a) **skips `resolveGeminiKey` entirely** — no 409 `KEY_REQUIRED` can lock out a user who never set up Gemini, the server key serves them — and (b) calls `reserveAiTextCall` with **`enforce: true` for EVERYONE**, not just managed-Gemini users. The pro-tier `count: 2` weighting is unchanged. Under `gemini` the metering rule stands byte-for-byte (managed → enforce, BYOK → no-op).

**What else changed on the Gemini path:** nothing — **except one deliberate upgrade**: the 4 fast routes (analyze, niche-ideas, ideas, hooks) now run their catch through the same shared `heroScriptLlmErrorResponse`, so a dead Gemini *fast* model answers **503 `MODEL_UNAVAILABLE`** instead of the old generic 500. That is the treatment the two pro routes already had; the alternative was leaving those 4 routes unable to report an exhausted OpenRouter balance as anything but a 500.

**Failure handling (no cross-provider fallback — ADR-0004's principle):** a dead model or an empty credit balance is reported honestly, never papered over by calling the other provider or the other tier. `src/lib/openrouter.ts` classifies failures and all 6 routes share one mapping (`heroScriptLlmErrorResponse`): 404 / "no endpoints found" / "not a valid model" → the existing **503 `MODEL_UNAVAILABLE`**; 402 / 429 → **503 `PROVIDER_CREDIT`** with the Thai copy *"ระบบ AI ไม่พร้อมใช้งานชั่วคราว (เครดิตผู้ให้บริการ)"*; 401 / 403 (server credential missing or rejected) → **503 `PROVIDER_UNAVAILABLE`** with the generic Thai unavailable copy **and never** an "API Key ใช้ไม่ได้" message, since the user has no OpenRouter key of their own to fix. Everything else keeps its existing path through `apiError`.

**Rollback (instant, no rebuild):** set `HERO_SCRIPT_PROVIDER=gemini` in `.env` and restart (`pm2 restart ai-content --update-env`). The Gemini path is untouched code, and `scripts/smoke-hero-script-live.ts` smokes BOTH providers on every run (each SKIPs cleanly when its key is absent) precisely so the rollback target is known-good, not assumed.
