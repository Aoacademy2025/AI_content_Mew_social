...
### 11.9 รอบ 8–13 (2026-09-07 คืน) → สูตร "V90" ล็อกโดยหูมิว และ port ชุดแรก

**โจทย์จาก prod clone ครั้งแรก (job 98cd2614):** (1) อ่านกระตุก (2) คำอังกฤษโดดเป็นสำเนียงอังกฤษ

**สาเหตุที่ยืนยันจากโค้ด:** `SENTENCE_END_RE` นับ `...`/`…`/`!` + ช่องว่างเป็นจบประโยค → "ปล่อย AI ทำงาน... " กลายเป็นชิ้น 17 ตัวอักษร + รอยต่อเงียบ 250 ms;
worker profile `thai-english-v13` เจนทุก Latin run ด้วย `language="English"` → คำนอกพจนานุกรม (Ask Me, Agent, Human Review, Draft, `→`) ไปเส้นทางอังกฤษ

**ผลฟัง (มิว):**
- r8 TLIT (ทับศัพท์ครบ + ไม่ตัดที่ `...`) ดีสุด แต่ "เสียงล้า/ต่างชาติ/อ่านผิด" → สมมติฐาน = คลิปอ้างอิง (ทุก job โคลนจากคลิปสอน 10 วิ ที่ลังเล)
- r9 reference ช่วงอื่นของการอัดเดิม (R30A) → "ธรรมชาติขึ้นมาก" แต่ "เร็ว รัว ไม่เว้นหายใจ" → **reference = คันโยกความเป็นธรรมชาติที่ใหญ่สุด**
- r10 ล้มทั้งรอบเพราะ selector เขียนไฟล์ประกอบทับไฟล์ reference ชื่อเดียวกัน (บทเรียน: ห้ามตั้งชื่อตระกูลซ้ำชื่อ reference) — ไม่ใช่เพราะ speed
- r11 (23 job): ทับศัพท์ *ก่อน* กฎ glue ให้ช่องว่างของผู้เขียนรอด = จุดหายใจ; ทุกตระกูลใช้ได้ "มาตรฐานดีขึ้น"; BP (reference R30B, speed 1.0) ถูกหมด; S85P (speed 0.85) จังหวะดีแต่ "Draft" เพี้ยน
- r12 speed 0.85 อ่าน Draft เป็น "ดาดราฟ" ทั้ง ดราฟต์/ดร๊าฟ (โมเดลแทรกพยางค์ในเสียงควบ ดร เมื่อช้าลง) → r13 ไล่ speed: **0.90 ผ่าน ("approve v90")**, 0.95 ยังเพี้ยน
- ชิ้นสั้น ~23 ตัวอักษร ไม่เสถียรทุกความเร็ว (จริง→จริงจริง / อ่านซ้ำทั้งประโยค) gate จับได้แต่เสียเงินเจนซ้ำ — ยังไม่แก้ (มิวให้ล็อก V90 ก่อน)

**สูตร V90 (ล็อก 2026-09-07):** reference = ช่วง 16.12–29.09 วิ ของคลิป 30 วิ (มิวอัปโหลดเองใน AI Studio) · speed 0.90 (worker ×1.4 = 1.26) · ทับศัพท์ก่อน glue · `...`/`…` ไม่ตัดประโยค · `!` ตัดแต่ไม่อ่าน · `→` = "ไป"

**port ชุดแรก (PR นี้):** normalizer `2026-09-08.2` — `normalizeSpeechPunctuation` + `applyThaiPronunciationDictionary` รันบนข้อความทั้งก้อนก่อน tokenize (คำอ่านติดกันข้างใน ช่องว่างรอบ ๆ ตามผู้เขียน), `NON_THAI_CHAR_RE` ไม่นับเครื่องหมายวรรคตอน (ทำให้ normalize ซ้ำเป็น no-op), `SENTENCE_END_RE` ไม่ตัดที่ `...`/`…`, พจนานุกรม +4 (Ask Me/Agent/Human Review/Draft), clone panel default 0.9× · fixtures 3 ตัว re-approve ช่องว่าง
**ยังไม่ทำ (PR ถัดไป ต้องออกแบบ):** auto-ทับศัพท์คำอังกฤษนอกพจนานุกรมด้วย Gemini + แก้ได้ในหน้า preflight; รวมชิ้นสั้นกับชิ้นข้าง ๆ; หูที่ 3 แบบเปรียบเทียบคู่; gate ยังจับ "ๆ" ท้ายประโยคไม่ได้ (r12 D2)
