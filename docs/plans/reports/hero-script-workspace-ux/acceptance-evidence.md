...
| AC7 | open-existing zero POST, save-before-create, one pending request, failure blocks handoff | PASS — active and non-active create actions share one page-owned handoff token; mounted delayed responses prove latest-save draining/failure preservation, brief confirmation, one competing POST, owned navigation and stale result rejection after New/open. |
