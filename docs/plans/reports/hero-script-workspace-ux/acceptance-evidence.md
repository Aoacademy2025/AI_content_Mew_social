...
| AC7 | open-existing zero POST, save-before-create, one pending request, failure blocks handoff | PASS — active and non-active creates share one immutable page-owned token; mounted delayed responses prove save draining, retry/cancel-only failures, exact-owner recovery, brief confirmation, one competing POST, success/failure ABA rejection, owned navigation and stale result rejection after New/open. |
