...
// One inline SVG bar chart per trend card (brief: no chart library). Per-day tooltip is a real
// React-state HTML element (a native SVG title element never surfaces on tap on iOS Safari /
// Android Chrome and has no keyboard path) — a transparent hit-rect per day drives it via pointer
// AND focus events, so touch and keyboard both reach it. No animation.
...
const tooltipStyle: React.CSSProperties = {
  background: "var(--ui-card-bg)", border: "1px solid var(--ui-card-border)", color: "var(--ui-text-primary)",
};
...
export function TrendBarChart({ dates, bars, overlay, layout, tooltipText, ariaLabel }: TrendBarChartProps) {
  ...
  // Tap (or click) anywhere outside the chart hides the tooltip; tapping the same bar toggles it
  // off (handled in the hit-rect's own pointerdown below).
  useEffect(() => {
    if (activeDay === null) return;
    const onOutside = (e: PointerEvent) => {
      if (!containerRef.current?.contains(e.target as Node)) setActiveDay(null);
    };
  ...
  const activeCxPercent = activeDay === null ? 0 : ((LEFT_MARGIN + activeDay * SLOT + SLOT / 2) / width) * 100;
    ...
    <div ref={containerRef} className="relative">
      {activeDay !== null && (
        <div
          role="status"
          aria-live="polite"
          className="pointer-events-none absolute z-10 whitespace-nowrap rounded px-2 py-1 text-[10px] shadow-sm"
          style={{ ...tooltipStyle, left: `${activeCxPercent}%`, top: 0, transform: "translate(-50%, -100%)" }}
        >
          {tooltipText(activeDay)}
        </div>
      )}
