              ...
              <div>
                ...
                <div className="grid gap-10 xl:grid-cols-[minmax(0,1fr)_300px]">
                  ...
                  <aside className="border-t border-[oklch(31%_0.025_300)] pt-6 xl:border-l xl:border-t-0 xl:pl-7 xl:pt-0">
                    ...
                    <div className="mt-7 border-t border-[oklch(31%_0.025_300)] pt-5">{selected.status === "needs_attention" ? <button type="button" disabled={saving} onClick={() => void decide("retry", "ลองทำงานเดิมอีกครั้งหลังตรวจสาเหตุแล้ว โดยคง asset และ generation epoch เดิม")} className="flex min-h-11 w-full items-center justify-center gap-2 border border-amber-400/60 text-sm font-semibold text-amber-100 hover:bg-amber-400/10 disabled:opacity-40"><CirclePlay className="h-4 w-4" />ลองงานเดิมอีกครั้ง</button> : selected.status === "paused" ? <button type="button" disabled={saving} onClick={() => void decide("resume")} className="flex min-h-11 w-full items-center justify-center gap-2 text-sm text-[oklch(79%_0.08_75)] hover:bg-[oklch(22%_0.025_300)]"><CirclePlay className="h-4 w-4" />ทำงานต่อ</button> : !["completed", "rendering"].includes(selected.status) ? <button type="button" disabled={saving} onClick={() => void decide("pause")} className="flex min-h-11 w-full items-center justify-center gap-2 text-sm text-[oklch(67%_0.025_300)] hover:bg-[oklch(22%_0.025_300)]"><CirclePause className="h-4 w-4" />พักโปรเจกต์</button> : null}</div>
                  </aside>
                </div>
              </div>
            ) : null}
