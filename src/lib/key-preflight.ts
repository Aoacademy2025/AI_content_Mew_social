 ...
 *    both Settings Test and job-submit preflight run the real TTS disambiguation probe
 *    (see `ElevenLabsCheckMode` below) because it's the only way to conclusively catch a
 *    scoped key that's missing text_to_speech itself — and the probe only ever costs the
 *    user anything when it SUCCEEDS, at which point the key was fine anyway. Settings
 *    used to treat `/v1/user` `missing_permissions` as valid without probing; that
 *    false-passed keys with no TTS permission (support ticket 2026-08-22).
 */
...
export interface ElevenLabsCheckOptions {
  timeoutMs?: number;
  /**
   * Both modes probe TTS on `/v1/user` `missing_permissions`. That 401 names
   * whatever permission `/v1/user` itself needs (`user_read`), never the caller's
   * actual use case — so it cannot tell a key that has text_to_speech-but-not-user_read
   * (fine) apart from one that has NEITHER. Only the real TTS endpoint can tell
   * them apart.
   *
   * "settings" (default) — Settings "Test key" button. Same probe as preflight so
   * a scoped key without text_to_speech cannot show a green check.
   *
   * "preflight" — job-submit gate. Fail-open still applies to network/timeout
   * (`verdict: "unknown"` does not block the job).
   *
   * Cost: the TTS probe is only ever PAID (spends the user's ElevenLabs character
   * quota) when it SUCCEEDS — a 401/403-rejected call generates no audio and bills
   * nothing. And if it succeeds, the key is fine and the job's real TTS step would
   * have spent that same ~1 character moments later anyway.
   */
 ...
 *     needs (user_read). Ambiguous re: text_to_speech — both modes probe TTS.
...
export async function testElevenLabsKey(key: string, opts: ElevenLabsCheckOptions = {}): Promise<KeyTestResult> {
  ...
  try {
    ...
    // Scoped key missing user_read (needed by /v1/user, not by us) is still ambiguous
    // for text_to_speech. Always probe TTS — Settings used to treat this as valid and
    // false-passed keys with no TTS permission.
    // Ambiguous 401 (any shape not resolved above) — only the real TTS endpoint can
    // conclusively confirm it. See `mode` doc above for the cost analysis that makes
    // it safe to always run this rather than skip it.
 ...
 *
 * `preflightElevenLabs` runs `testElevenLabsKey` with `mode: "preflight"`: it runs the
 * real TTS probe on an ambiguous 401 — see the `ElevenLabsCheckMode` doc above for why
 * this is safe (the probe only ever costs the user anything when it succeeds).
