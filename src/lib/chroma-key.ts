...
/**
 * The chroma-key + despill + alpha-feather chain, applied to the avatar stream AFTER it has been
 * scaled to its final display resolution. `format=yuva444p` forces full-chroma (4:4:4) keying so
 * edges aren't halved by the source's 4:2:0 subsampling. The feather erodes the matte 1px and
 * blurs it — but ONLY on the alpha plane (erosion threshold0..2=0 leave Y/U/V untouched; gblur
 * planes=8 = alpha only), so the subject's color/detail is preserved while the edge softens.
 *
 * Every param is re-sanitized here so no unsafe value can reach the ffmpeg args regardless of caller.
 */
export function buildKeyChain(params: ChromaParams): string {
 ...
 */
export function buildCompositeFilter(params: ChromaParams, layout: AvatarLayout | null): string {
  const keyChain = buildKeyChain(params);
  if (layout) {
