...
- `src/app/(dashboard)/` — pages: pricing (redesigned + founding), settings, video-creator, video-editor, content, style, admin — split into routes (ADR 0062): /admin overview + /admin/revenue, users, support, coupons, insights, loanwords, settings, storage, music, updates
- `src/app/api/` — ~87 routes (`videos/*`, `payments/*`, `coupons/*`, `founding/*`, `cron/*`, `heygen/*`, `elevenlabs/*`, …)
- `src/remotion/` — render compositions
...
- **Config shadowing (HISTORICAL — fixed 06-16, commit `5fb76cb`):** `next.config.js` used to SHADOW `next.config.ts` (Next 15 resolves .js first) → everything in `next.config.ts` was inactive. `next.config.js` was deleted; `next.config.ts` is now the sole active config (serverExternalPackages, OOM `cpus:1`, webpack externals, `/renders` rewrite, `ignoreBuildErrors` all live). If a `next.config.js` ever reappears, it will shadow `.ts` again — treat that as a bug. Likewise `ecosystem.config.js` `env:` block shadows `.env` for `RENDER_*`/cache, and a plain `pm2 restart` keeps the OLD env → use `pm2 restart <app> --update-env`. **But `--update-env` only refreshes the calling shell's environment — it replays the app definition PM2 already SAVED, so a CHANGED `env:` block in `ecosystem.config.js` never reaches the process.** After editing an app's `env:` block you must re-register it: `pm2 delete <app> && pm2 start ecosystem.config.js --only <app> --update-env && pm2 save`. This bit us on 2026-09-13: C9's nightly R2 backup looked dead (`BACKUP_R2 not configured`) purely because `db-backup` had been restarted by name instead of re-registered.
...
- **Subtitle timing (06-12, PRs #35-#39):** ซับของเสียง TTS (Gemini/ElevenLabs) มาจาก `timing` ใน TTS response — exact-by-arithmetic, **ข้าม transcribe** (`src/lib/tts-timing.ts` + `_components/tts-timing-captions.ts`); การ์ด viral มาจาก `/api/videos/split-script` (text-only LLM, server validate ห้ามแก้ข้อความ). transcribe = fallback สำหรับ avatar/อัปโหลด เท่านั้น. ทุกชั้นมี fail-open → ห้าม "ซ่อม" โดยเอา transcribe กลับมาเป็น path หลัก. **Update 08-31 (ADR 0056, supersedes the gate half of this rule):** สำหรับ Gemini / Hero AI Voice, `alignNarrationOnce` ใน `orchestrator.ts` ยิง transcribe **1 ครั้ง** ภายใน `SUBTITLE_VERIFY_BUDGET_MS` — สำเร็จ → `forced_alignment` เป็น render clock (แม่นกว่า: การ์ด >1 s เพี้ยน 11 % → 1.3 %), ล้ม/หมดเวลา → `tts_segment_timing` → `avatar_script_clock`; **ไม่มี** regen TTS, ไม่มี retry, export ไม่ re-align, QA ซับเป็น *report* (`warning`) — บล็อกได้แค่ `empty_script`/`empty_captions`. อย่าถอด `alignNarrationOnce` ออกและอย่าใส่ gate กลับ.
- **Hero Voice clone on prod (09-07, ADR 0061):** clone transport is fail-closed unless `.env` has `HERO_VOICE_CLONE_PRODUCTION=1` (owner-consent gate, Mew's own voice only; never together with `HERO_VOICE_CANARY_*`). Without it AI Studio says "Hero Voice clone ยังไม่พร้อมใช้งาน" even with the five `RUNPOD_HERO_VOICE_CLONE_*` inputs set. Rollback = unset that one var + `pm2 restart ai-content --update-env`. Runbook: `docs/ops/hero-voice-clone-rollout.md` (last section). **The RunPod template must use the boot script from that runbook, NOT the image default CMD** — default CMD = every job `{"ok": false}` in 0.7 s (`CLONE_OUTPUT_INVALID`, refunded). ASR gate (`HERO_VOICE_ASR_GATE=1`) checks dropped AND inserted runs (จริง→จริงๆ) — don't loosen it to make a stubborn sentence pass; fix the script text instead.
...
- **Clerk middleware matcher must whitelist static media** (`mp4|webm|mov` in `src/proxy.ts` — Next 16 renamed the middleware file) or those files get redirected to /login (symptom: poster `.jpg` loads but `<video>` won't play). Any new static media type needs the same.
- **BYOK by default:** paid external-provider features need the user's own API keys → onboarding must guide key setup. Approved managed exceptions are narrow and product-funded: OmniVoice audio (server credential, HTTPS/IP restriction, fail-closed flags + allowlist, backend AI-audio/minute limits; ADR 0003) Hero AI Image/Brand Visual generation through the qualified RunPod Z-Image route (shared-credit or Starter AI Image Allowance settlement, fixed per-image price, fail-closed rollout and cost guard); and the **managed stock library** — the team's Pexels/Pixabay keys serve any account that has not set its own, behind `MANAGED_STOCK` (ADR 0025). Stock is the one managed exception that costs no money (both APIs are free), so it is bounded by rate limits instead: BYOK always wins when a key exists, Pixabay is tried before Pexels, searches are cached 24h, and per-job / hourly / monthly ceilings fail closed to key-free photo providers and AI images rather than failing the render. No exception may silently fall back to another AI Engine.
- Windows-aware (MAX_PATH, ffmpeg installer); render tuned for low-RAM hosts.
- **Admin IA is fixed by ADR 0062:** each admin surface answers one question and **money renders only on `/admin/revenue`** — `/admin` is a cheap overview (trend cards, no disk walk, no settings read) and a new money card goes to `/admin/revenue` even when another page would be convenient. See `docs/adr/0062-each-admin-surface-answers-one-question.md`.
